// Let's make the artists page slightly more user friendly
...
(function ($) {
	...
	function addToHead(type, src) {
		...
		if (type === 'js') { t.src = src; }
		if (type === 'css') { t.appendChild(document.createTextNode(src)); }
 		h.appendChild(t);
	}
	...
	// wait until hunch script loads to invoke hunch
	window.setTimeout(function async() {
		if (typeof $target.hunch === 'function') { $target.hunch(data); }
		else { window.setTimeout(async, 50); }
	}, 50);
